function Perceptron(opts) {
  ...
  // If the perceptron is declared globally, return a new perceptron

  if (!opts) opts = {};
  // If thare are no options input, default options to an array

  var debug = 'debug' in opts ? opts.debug : false;
  // if 'debug' is present in the options, start debugging

  var weights = 'weights' in opts
    ? opts.weights.slice()
    : []
  // if there are weight present int 
